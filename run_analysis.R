...
testData <- read.fwf(paste0(workingDir,"UCI HAR Dataset\\test\\X_test.txt"), widths = rep(16, 561))
testSubject <- read.table(paste0(workingDir,"UCI HAR Dataset\\test\\subject_test.txt"))
testActivity <- read.table(paste0(workingDir,"UCI HAR Dataset\\test\\y_test.txt"))
...
trainData <- read.fwf(paste0(workingDir,"UCI HAR Dataset\\train\\X_train.txt"), widths = rep(16, 561))
trainSubject <- read.table(paste0(workingDir,"UCI HAR Dataset\\train\\subject_train.txt"))
trainActivity <- read.table(paste0(workingDir,"UCI HAR Dataset\\train\\y_train.txt"))
...
activityTable <- read.table(paste0(workingDir,"UCI HAR Dataset\\activity_labels.txt"), col.names = c("activityid","activityname"))
